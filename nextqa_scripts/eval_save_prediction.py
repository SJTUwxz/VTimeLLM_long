...
def parse_args():
    ...
    parser.add_argument("--test_samples", type=int, default=-1, required=False)
...
if __name__ == "__main__":
    ...
    video = {}
    stop = args.test_samples
    for index, data in tqdm(enumerate(js)):
        if stop > 0:
            if index == stop: break
        ...
        if args.task in ['nextqa']:
            ...
            converted_groundtruth = []
            for k, v in groundtruths.items():
                converted_groundtruth.append(convert(duration, v))

            prediction = inference(model, features, question, tokenizer, do_sample=False)
            print(converted_groundtruth, prediction)
            # print(prediction)

            video[id][qid]['prediction'] = prediction 
