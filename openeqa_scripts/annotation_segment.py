...
def frames2segment(duration, frames, out_segment_gap=2, segment_length_limit=5):
    ...
    for i, frm in enumerate(frames):
        if i == 0:
            ...
            continue
        # if frame distance between current frame and last frame > out_segment_gap, create new segment
        # if frame distance between current frame and first frame > in _segment_gap, create new segment
        if curr_group[-1] - curr_group[0] > segment_length_limit:
            groups.append([curr_group[0], min(frames[-1], curr_group[-1])])
            curr_group = [frm]
            continue


        if frm - curr_group[-1] > out_segment_gap :
            if len(curr_group) == 1:
                # throw away the segment with only one frame
                curr_group.append(curr_group[0])
            groups.append([curr_group[0], min(frames[-1], curr_group[-1])])
            curr_group = [frm]
        else:
            curr_group.append(frm)
    if curr_group:
        if len(curr_group) == 1:
            curr_group.append(curr_group[0])
        groups.append([curr_group[0], min(frames[-1], curr_group[-1])])
    assert len(groups) > 0, f"no segment found: {frames}"
    return groups
...
if __name__ == "__main__":
    ...
    save_f = f"train-segment"
    val_save_f = f"val-segment"
