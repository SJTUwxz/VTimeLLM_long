...
def parse_args():
    ...
    parser.add_argument("--missing_prediction_complete", type=str, default='random', choices=["random", "reuse"])
        ...
        if args.task in ['custom']:
            ...
            for frm in predicted_frames:
                try:
                    ...
                    continue
            if args.missing_prediction_complete == "random":
                while len(result) < 6:
                    result.append(random.randint(0, int(duration)))
            elif args.missing_prediction_complete == "reuse":
                if len(result) < 6:
